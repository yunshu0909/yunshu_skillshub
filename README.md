...
> 想看多个 skills 如何组成一条完整开发链路？查看 [CodePal 托管 Coding 项目案例](https://github.com/yunshu0909/codepal-managed-project-example)。该案例由 [CodePal](https://github.com/yunshu0909/CodePal)「新建项目」生成，演示 Issue → 设计 → PRD → 测试 → PR → 反馈的完整流程。

### 📊 Skills 总览

目前共包含 **33 个 Skills**，分为以下 3 大类：
...
| [Issue 池全生命周期](./issue-pool) | 想法入池、关联检查、拆 task 与滚动 plan |
...
#### 🧩 [Issue 池全生命周期](./issue-pool) (Issue Pool)
**描述**: 托管 Coding 的规划入口。把零散想法、痛点和外部反馈记入 `ISSUES.md`，检查同源关系，再收敛成一个版本能交付、可验收的 task；复杂事项用滚动 plan 分批吐 task

**适用场景**:
- 随手记录并检查重复或相邻需求
- 把模糊 Issue 讨论成可开工 task
- 为复杂方向编写框架计划和下一批任务
- 维护 Issue → task → specs → 开发的可追溯关系
...
完整用法见 [CodePal 托管 Coding 项目案例](https://github.com/yunshu0909/codepal-managed-project-example)。
...
│   └── SKILL.md               # Skill 定义文件
├── issue-pool/                 # Issue 池全生命周期
│   ├── SKILL.md               # Skill 定义文件
│   ├── references/            # 滚动 plan 写作规范
│   └── tools/                 # Markdown 转 HTML 工具
...
> See how multiple skills form one end-to-end workflow in the [CodePal Managed Project Example](https://github.com/yunshu0909/codepal-managed-project-example), generated by [CodePal](https://github.com/yunshu0909/CodePal).

### 📊 Skills Overview

Currently includes **33 Skills** across 3 categories:
...
| [Issue Pool](./issue-pool) | Capture ideas, check relationships, split tasks, and roll plans forward |
...
#### 🧩 [Issue Pool](./issue-pool)
**Description**: The planning entry point for managed coding. It captures ideas and feedback in `ISSUES.md`, checks related items, turns clear work into verifiable tasks, and uses rolling plans for work that cannot fit in one release.

**Use Cases**:
- Capture ideas while checking duplicates and adjacent needs
- Refine a vague issue into a ready-to-build task
- Write a framework plan and define only the next delivery batch
- Preserve traceability from Issue to task, specs, and implementation

**Triggers**:
```
record an issue / split issue
review the issue pool / write a framework plan
/issue-pool
```

See the complete workflow in the [CodePal Managed Project Example](https://github.com/yunshu0909/codepal-managed-project-example).
...
├── backlog-manager/            # Backlog Manager
│   └── SKILL.md               # Skill definition file
├── issue-pool/                 # Issue Pool lifecycle
│   ├── SKILL.md               # Skill definition file
│   ├── references/            # Rolling-plan writing guide
│   └── tools/                 # Markdown-to-HTML tools
